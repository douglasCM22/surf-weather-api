...
export default class StormGlass {
    ...
    public async fetchPoints(lat: number, lng: number): Promise<StormGlassNormalizedForecastPoint[]> {
        try {
            const resp = await this.request.get<StormGlassForecastResponse>(`${stormGlassResourceConfig.get('apiUrl')}/weather/point?lat=${lat}&lng=${lng}&params=${this.paramsAPI}&source=${this.sourceAPI}&end=${this.endAPI}`, {
                headers: {
