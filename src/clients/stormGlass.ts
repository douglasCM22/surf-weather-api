...
export default class StormGlass {
    ...
    public async fetchPoints(
        lat: number,
        lng: number
    ): Promise<StormGlassNormalizedForecastPoint[]> {
        try {
            const resp = await this.request.get<StormGlassForecastResponse>(
                `${stormGlassResourceConfig.get('apiUrl')}/weather/point?lat=${lat}&lng=${lng}&params=${this.paramsAPI}&source=${this.sourceAPI}&end=${this.endAPI}`,
                {
                    headers: {
    ...
    private normalizeResponse(
        data: StormGlassForecastResponse
    ): StormGlassNormalizedForecastPoint[] {
        return data.hours
            .filter(this.isValidPoint.bind(this))
            .map<StormGlassNormalizedForecastPoint>((hour) => {
                return {
                    time: hour.time,
